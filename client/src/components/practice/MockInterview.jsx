...
const MockInterview = () => {
    ...
    const generateFinalReport = async (finalQuestions) => {
        setLoading(true);
        try {
            const reportResponse = await axios.post("https://skillup-ai-powered-learning.onrender.com/api/mock/generate-report", {
    ...
    const evaluateAnswer = async (answer, behaviorScore) => { 
        ...
        try {
            const evaluationResponse = await axios.post("https://skillup-ai-powered-learning.onrender.com/api/mock/evaluate-answer", {
    ...
    const startInterview = async () => {
        ...
        try {
            const response = await axios.post("https://skillup-ai-powered-learning.onrender.com/api/mock/generate-interview", { role });
...
useEffect(() => {
    if (interviewStarted && currentQuestion) {
        // Speak the question only once per question index
        speak({ text: currentQuestion.text });
    } 
    else if (report) {
        speak({ text: "The interview is complete. Preparing your final feedback report now." });
    }
    // Run only when the question changes or report is generated
    // ✅ remove 'speaking' and 'listening' from dependencies
}, [currentQIndex, interviewStarted, report]);
